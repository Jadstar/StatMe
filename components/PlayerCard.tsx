...
const PlayerCard: React.FC<PlayerCardProps> = ({ playerName, overallRating, categoryScores, imageUrl, onShare }) => {
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'StatMe Player Card',
      text: `Check out my StatMe card for ${playerName}! Overall Rating: ${overallRating}. Create yours on StatMe!`,
    ...
    try {
      ...
      // Attempt to copy to clipboard as a further fallback if share fails
      try {
        await navigator.clipboard.writeText(`${shareData.text} ${shareData.url}`);
          ...
          return (
            <div key={categoryId} className="bg-slate-700/50 p-4 rounded-lg shadow-md flex items-center justify-between">
              <div className="flex items-center">
                {IconComponent && <IconComponent className={`w-6 h-6 mr-3 ${color}`} />}
